/* planExtract.js — a geometry-only CAD floor-plan SVG -> the plan JSON the 3D engine renders.
 *
 * PURE. No fs, no process, no DOM: text in, plan object out. That is what lets the
 * SAME code run in the browser (so a facilities user can import a plan without a
 * terminal) and in tools/extract-plan.mjs, which is now a thin CLI over this file.
 * One implementation, two front-ends — the alternative was two copies of a raster
 * flood-fill drifting apart.
 *
 * The exports carry two things that make this reliable:
 *   1. a <metadata id="plan-plate"> block with world bounds + px-per-world-unit, so
 *      pixels convert back to real millimetres, and
 *   2. data-layer attributes carrying the original CAD layer names, so wall / door /
 *      glazing / furniture geometry can be told apart instead of guessed at.
 *
 * Paths are pure polylines (M/L/Z only) with no transforms, so parsing is exact — no
 * curve flattening, no matrix stack.
 *
 * Everything not on a recognised layer is DROPPED and reported, so nothing is silently
 * kept or lost. Hatch fills and glazing mullion strokes are the bulk of these files
 * (110k paths in one) and carry no useful structure at 3D viewing scale.
 *
 * Kept as JavaScript with a hand-written .d.ts for the same reason as buildEstate.js:
 * it is dense numeric code guarded by fixture checks, and Node has to import it.
 */
...
/** Thrown for input we can name a reason for — surfaced to the user verbatim. */
export class PlanExtractError extends Error {}

/**
 * @param {string} svg   the SVG document text
 * @param {{id?:string,name?:string,source?:string,cell?:number,minArea?:number,seal?:number}} opts
 */
export function extractPlan(svg, opts = {}) {
  const cell = opts.cell ?? 0.05;
  const minArea = opts.minArea ?? 2.5;
  const seal = opts.seal ?? 0.6;

  const metaMatch = /<metadata id="plan-plate">([\s\S]*?)<\/metadata>/.exec(svg);
  if (!metaMatch) {
    throw new PlanExtractError(
      'This SVG has no <metadata id="plan-plate"> block, so there is no way to convert ' +
        'its pixels back to real metres. Re-export the plan with plate metadata.',
    );
  }
  let meta;
  try {
    meta = JSON.parse(decode(metaMatch[1]));
  } catch {
    throw new PlanExtractError('The plan-plate metadata is not valid JSON.');
  }
  const pxPerM = meta?.transform?.px_per_unit_x * 1000; // world units are mm
  const pxPerMy = meta?.transform?.px_per_unit_y * 1000;
  if (!Number.isFinite(pxPerM) || !Number.isFinite(pxPerMy) || pxPerM <= 0 || pxPerMy <= 0) {
    throw new PlanExtractError('The plan-plate metadata has no usable px_per_unit scale.');
  }
  ...
  const gRe = /<g\s+data-layer="([^"]*)"[^>]*>([\s\S]*?)<\/g>/g;
  let g;
  while ((g = gRe.exec(svg))) {
    ...
    if (!role) {
      const key = role === undefined ? layer : `${layer} (annotation)`;
      dropped.set(key, (dropped.get(key) || 0) + paths.length);
      continue;
    }
  ...
  if (!byRole.walls.length && !byRole.glazing.length) {
    throw new PlanExtractError(
      'No wall or glazing geometry was found on any recognised CAD layer. Check that ' +
        'the export keeps its data-layer names.',
    );
  }

  /* plate extents come from structure (walls first), not the drawing border/title block */
  ...
  if (!(widthM > 0.5) || !(depthM > 0.5)) {
    throw new PlanExtractError('The extracted plate is smaller than a metre — the scale looks wrong.');
  }
  ...
  const rooms = detectRooms(layers, widthM, depthM, cell, minArea, seal);

  return {
    plan: {
      id: opts.id ?? 'plan',
      name: opts.name ?? 'Floor plan',
      source: opts.source ?? '',
    ...
    /** Reported, never silent: which CAD layers were used and which were discarded. */
